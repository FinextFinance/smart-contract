...
contract Vester is IVester, IERC20, ReentrancyGuard, Governable {
    ...
    uint256 public  BURN_RATE_PRECISION = 970000;
    string public name;
    ...
    function setBurnRate(uint256 _burn_rate ) external onlyGov {
       BURN_RATE_PRECISION = _burn_rate;
    }
    ...
    function withdrawToken(address _token, address _account, uint256 _amount) external onlyGov {
        require(
                _token != esToken && _token != claimableToken ,
                "!token"
            );
    ...
    function _exchange(address _account, uint256 _amount) private {
        require(_amount > 0, "Exchange: invalid _amount");
        IBasisAsset(esToken).burnFrom(_account, _amount);
        uint256 _burn_amount =  _amount.mul(BURN_RATE_PRECISION).div(RATE_PRECISION);
        require(_burn_amount < _amount,"invalid burn amount!");
        uint256 _ramound = _amount.sub(_burn_amount);
        IBasisAsset(claimableToken).burn(_burn_amount);
        IERC20(claimableToken).safeTransfer(_account, _ramound);

         emit Exchange(_account, _amount);
    }
