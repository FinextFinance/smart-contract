...
contract GenesisMultiRewardPool is IMultiRewardPool, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. MAINTOKENs to distribute in the pool.
        uint256 lastRewardTime; // Last time that MAINTOKENs distribution occurred.
        uint256 accTokenPerShare; // Accumulated MAINTOKENs per share, times 1e18. See below.
    ...
    uint256 public totalRewards = 50000 ether;


    // Time when pools ends.
    uint256 public poolEndTimes;

    uint256 public tokenoPerSecond;

    uint256 public constant REWARD_RATE_DENOMINATION = 1000000;
    uint256 public constant REWARD_RATE_ESTOKEN = 9000000;
    ...
    constructor(
        ...
        poolEndTimes = poolStartTime + 14 days; // 2 week

        tokenoPerSecond = totalRewards.div(14 days);
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "TokenoRewardPool: caller is not the operator"
        );
        _;
    }
        ...
        returns (address[] memory _rewards)
    {
        ...
        _rewards[0] = address(maintoken);
        _rewards[1] = address(estoken);
    }
    ...
    function getRewardPerSecond() external view override returns (uint256) {
            if (block.timestamp <= poolEndTimes)
                return tokenoPerSecond;
        return 0;
    }
        ...
        returns (uint256[] memory _rewardPerSecondArr)
    {
        _rewardPerSecondArr = new uint256[](2);
            if (block.timestamp <= poolEndTimes) {
                _rewardPerSecondArr[0] = tokenoPerSecond;
                _rewardPerSecondArr[1] = tokenoPerSecond
                    .mul(REWARD_RATE_ESTOKEN)
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(
                ...
                "maintokenRewardPool: existing pool?"
            );
        }
    }
    ...
    // Update the given pool's maintoken locked time. Can only be called by the owner.
        ...
        returns (uint256)
    {
        ...
        if (_toTime >= poolEndTimes) {
            if (_fromTime >= poolEndTimes) return 0;
            if (_fromTime <= poolStartTime)
                return poolEndTimes.sub(poolStartTime).mul(tokenoPerSecond);
            return poolEndTimes.sub(_fromTime).mul(tokenoPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime)
                return _toTime.sub(poolStartTime).mul(tokenoPerSecond);
            return _toTime.sub(_fromTime).mul(tokenoPerSecond);
        }


    }

    // View function to see pending MAINTOKENs on frontend.
    function pendingReward(uint256 _pid, address _user)
        public
        view
        override
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _maintokenReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accTokenPerShare = accTokenPerShare.add(
                _maintokenReward.mul(1e18).div(tokenSupply)
            );
        }
    ...
    {
        uint256 _maintokenReward = pendingReward(_pid, _user);
        _pendingMultiRewardArr = new uint256[](2);
        _pendingMultiRewardArr[0] = _maintokenReward;
        _pendingMultiRewardArr[1] = _maintokenReward.mul(REWARD_RATE_ESTOKEN).div(
    ...
    {
        uint256 _maintokenTotalReward = pendingAllRewards(_user);
        _totalMultiRewardArr = new uint256[](2);
        _totalMultiRewardArr[0] = _maintokenTotalReward;
        _totalMultiRewardArr[1] = _maintokenTotalReward.mul(REWARD_RATE_ESTOKEN).div(
            REWARD_RATE_DENOMINATION
        );
    ...
    function _claimReward(address _account, uint256 _maintokenAmt) internal {
        uint256 _estokenAmt = _maintokenAmt.mul(REWARD_RATE_ESTOKEN).div(
            REWARD_RATE_DENOMINATION
        );

        _safeTokenTransfer(maintoken, _account, _maintokenAmt);
        _safeTokenTransfer(estoken, _account, _estokenAmt);
        emit RewardPaid(_account, _maintokenAmt, _estokenAmt);
    }

    // Safe maintoken transfer function, just in case if rounding error causes pool to not have enough MAINTOKENs.
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTimes + 90 days) {
            // do not allow to drain token if less than 90 days after farming
            require(
                _token != maintoken && _token != estoken ,
                "reward"
            );
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
